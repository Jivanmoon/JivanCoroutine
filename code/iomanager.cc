...
#undef XX
    ...
    IOManager::FdContext::EventContext &IOManager::FdContext::getEventContext(IOManager::Event event) {
        switch (event) {
            ...
            case IOManager::WRITE:
                return write;
            default:
                JIVAN_ASSERT2(false, "getContext");
        }
    ...
    int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
        ...
        if (static_cast<int>(m_fdContexts.size()) > fd) {
            ...
            while (static_cast<int>(m_fdContexts.size()) <= fd) contextResize(fd * 2);
            fd_ctx = m_fdContexts[fd];
        }

        // 同一个fd不允许重复添加相同的事件
        FdContext::MutexType::Lock lock2(fd_ctx->mutex);
        if (JIVAN_UNLIKELY(fd_ctx->events & event)) {
            JIVAN_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
                                      << " event=" << (EPOLL_EVENTS) event
                                      << " fd_ctx.event=" << (EPOLL_EVENTS) fd_ctx->events;
            JIVAN_ASSERT(!(fd_ctx->events & event));
        }
        ...
        if (rt) {
            JIVAN_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
        ...
        JIVAN_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
        ...
        if (cb) {
            ...
            JIVAN_ASSERT2(event_ctx.fiber->getState() == Fiber::RUNNING, "state=" << event_ctx.fiber->getState());
        }
        return 0;
    }
 ...
 */
    void IOManager::tickle() {
        JIVAN_LOG_DEBUG(g_logger) << "tickle";
        if (!hasIdleThreads()) {
            return;
        }
 ...
 */
    void IOManager::idle() {
        ...
        while (true) {
            ...
            if (JIVAN_UNLIKELY(stopping(next_timeout))) {
                JIVAN_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
                break;
            }
