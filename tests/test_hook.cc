 ...
 */
void test_sleep() {
    JIVAN_LOG_INFO(g_logger) << "test_sleep begin";
    jivan::IOManager iom;
    ...
    iom.schedule([] {
        sleep(2);
        JIVAN_LOG_INFO(g_logger) << "sleep 2";
    });

    iom.schedule([] {
        sleep(3);
        JIVAN_LOG_INFO(g_logger) << "sleep 3";
    });

    JIVAN_LOG_INFO(g_logger) << "test_sleep end";
}
...
void test_sock() {
    ...
    JIVAN_LOG_INFO(g_logger) << "begin connect";
    int rt = connect(sock, (const sockaddr*)&addr, sizeof(addr));
    JIVAN_LOG_INFO(g_logger) << "connect rt=" << rt << " errno=" << errno;
    ...
    JIVAN_LOG_INFO(g_logger) << "send rt=" << rt << " errno=" << errno;
    ...
    JIVAN_LOG_INFO(g_logger) << "recv rt=" << rt << " errno=" << errno;
