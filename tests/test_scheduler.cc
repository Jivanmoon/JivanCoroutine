 ...
 */
void test_fiber1() {
    JIVAN_LOG_INFO(g_logger) << "test_fiber1 begin";
  
    /**
     * 协程主动让出执行权，在yield之前，协程必须再次将自己添加到调度器任务队列中，
     * 否则yield之后没人管，协程会处理未执行完的逃逸状态，测试时可以将下面这行注释掉以观察效果
     */
    jivan::IOManager::GetThis()->schedule(jivan::Fiber::GetThis());
    JIVAN_LOG_INFO(g_logger) << "before test_fiber1 yield";
    jivan::Fiber::GetThis()->yield();

    JIVAN_LOG_INFO(g_logger) << "after test_fiber1 yield";
    JIVAN_LOG_INFO(g_logger) << "test_fiber1 end";
}
 ...
 */
void test_fiber2() {
    JIVAN_LOG_INFO(g_logger) << "test_fiber2 begin";
    ...
    JIVAN_LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
    JIVAN_LOG_INFO(g_logger) << "test_fiber3 begin";
    JIVAN_LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
    static int count = 0;

    JIVAN_LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
    JIVAN_LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
 ...
 */
void test_fiber4() {
    JIVAN_LOG_INFO(g_logger) << "test_fiber4 begin";
    
    for (int i = 0; i < 3; i++) {
        jivan::IOManager::GetThis()->schedule(test_fiber5, jivan::GetThreadId());
    }

    JIVAN_LOG_INFO(g_logger) << "test_fiber4 end";
}
...
//     code::IOManager iom(3, false);
    ...
    jivan::Fiber::ptr fiber(new jivan::Fiber(&test_fiber3));
    iom.schedule(fiber);
    ...
    /**
     * 只要调度器未停止，就可以添加调度任务
     * 包括在子协程中也可以通过jivan::Scheduler::GetThis()->scheduler()的方式继续添加调度任务
     */
    iom.schedule(test_fiber4);
